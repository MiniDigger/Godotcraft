...
public class Console : CanvasLayer {
	...
	[Export] public String action_console_toggle = "console_toggle";
	...
	public override void _Input(InputEvent @event) {
		if (Input.IsActionJustPressed(action_console_toggle)) {
			toggleConsole();
		}
	}

	public Command getCommand(String name) {
		return _rootGroup.getCommand(name, new List<string>());
	}

	public CommandBuilder addCommand(String name, Action<object> target) {
	...
	public void _toggleAnimationFinished() {
		if (!isConsoleShown) {
			_consoleBox.Hide();
		}
	}
